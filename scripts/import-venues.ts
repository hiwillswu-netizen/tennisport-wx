/**
 * 球馆数据导入脚本
 * 使用方式: npx tsx scripts/import-venues.ts
 */

import cloudbase from '@cloudbase/js-sdk';

const ENV_ID = 'willswu-d1gttwhxle25af194';
const PUBLISHABLE_KEY = 'eyJhbGciOiJSUzI1NiIsImtpZCI6IjlkMWRjMzFlLWI0ZDAtNDQ4Yi1hNzZmLWIwY2M2M2Q4MTQ5OCJ9..OyblpO2etOYzh_OCxskZc3uVaBAvDX0O0XVkenrNlNOaxbmwTOnRaMAp3poIfsHsiHLjIi3YBhR4g-83qfENwChJC5QbvRy1tncItkUPG-kBMz8GRd7lpEk13CFqUZLHBUQ_cyqUCuqGFY-qSIHiBhocVImiiK8iYQvY71iMBoCu2dfP0cOJDuAmjN088-yIGrUBpSgapuWjXMxt6yOUGCBCIZ9WfOJ4be_jo2XUQS6psJ53Z5FAe7chz62PszBv9mbVuQ_hS15V0yhY2E0p_oyTYnEH1B53s6zPyn7g8-9zcanr_n-wg8d-zkQuOxOb7IaqeGWMG_rMXLwB7Dm-qQ';
...
// 转换为数据库格式
function transformVenue(venue: typeof venuesData[0], index: number) {
...
async function main() {
  ...
  // 先检查是否已有数据
  const existingData = await collection.count();
  console.log(`当前数据库中有 ${existingData.total} 条记录`);
  
  if (existingData.total > 0) {
    console.log('数据库已有数据，跳过导入。如需重新导入，请先清空数据库。');
    return;
  }
  ...
  for (let i = 0; i < venuesData.length; i++) {
    ...
    const dbVenue = transformVenue(venue, i);
    
    try {
      await collection.add(dbVenue);
      successCount++;
      console.log(`[${i + 1}/${venuesData.length}] 导入成功: ${venue.name}`);
    } catch (error) {
      failCount++;
      console.error(`[${i + 1}/${venuesData.length}] 导入失败: ${venue.name}`, error);
    }
  }
